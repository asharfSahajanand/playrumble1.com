...
(function (root, factory) {
    ...
    (function () {
        ...
        function fixAttribute(element, attrName) {
            ...
            if (!value) return;
            if (value.indexOf(brokenPrefix) === 0) {
                element.setAttribute(attrName, fixedPrefix + value.substring(brokenPrefix.length));
            }
        }

        var images = document.getElementsByTagName("img");
        for (var i = 0; i < images.length; i++) {
            fixAttribute(images[i], "src");
            fixAttribute(images[i], "data-src");
            fixAttribute(images[i], "srcset");
            fixAttribute(images[i], "data-srcset");
        }
    })();
